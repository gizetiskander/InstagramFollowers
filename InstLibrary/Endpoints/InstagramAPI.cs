using InstLibrary.Extensions;
using InstLibrary.Models.Responses;
...
namespace InstLibrary.Endpoints
{
    ...
    public abstract class InstagramApi
    {
        ...
        /// </value>
        public InstApi InstApi{ get; private set; }
        ...
        internal InstagramApi(string endpoint, InstApi instApi)
            : this(endpoint, instApi, null)
        {
        ...
        /// <param name="instApi">The instagram configuration.</param>
        /// <param name="oauthResponse">The oauth response.</param>
        protected InstagramApi(string endpoint, InstApi instApi, OAuthResponse oauthResponse)
        {
            InstApi = instApi;
            ...
            Client = new HttpClient(handler) { BaseAddress = new Uri(new Uri(InstApi.InstUri), endpoint) };
        }
        ...
        {
            if (EnforceSignedRequests && !String.IsNullOrWhiteSpace(InstApi.ClientSecret))
            {
        ...
        private string CreateRequestSignature(HttpRequestMessage request)
        {
            ...
            var hash = Hash(InstApi.ClientSecret, valueToHash);
